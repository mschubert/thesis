...
do = function(fid, caption="", ...) {
    file.path(speed2, "drug_assocs/assocs_all", paste0(fid, ".RData")) %>%
        load2("assocs.pan") %>%
        transmute(Drug = drug,
                  Pathway = ifelse(nchar(scores) > 30,
                                   paste0(substr(scores, 1, 27), "..."),
                                   scores),
                  Size=size, Effect=estimate, `P-value`=p.value, FDR=adj.p) %>%
        tab(caption=paste(caption, "vs. drugs (pan-cancer)"), p=0.05, n=50, ...)
}
...
do = function(fid, caption="", ...) {
    file.path(speed2, "tcga_pathway_per_mutation/assocs_driver_mapped", paste0(fid, ".RData")) %>%
        load2() %>%
        filter(subset == "pan") %>%
        transmute(Mutation=m, Pathway=scores, Size=size, Effect=estimate, `P-value`=p.value, FDR=adj.p) %>%
        tab(caption=paste(caption, "vs. mutations (pan-cancer)"), p=0.05, n=50, ...)
}
do("gsva_go", "Gene Ontology")
do("gsva_reactome", "Reactome")
do("spia", "SPIA")
...
do = function(fid, caption="", ...) {
    file.path(speed2, "tcga_pathway_per_mutation/assocs_cna_mapped", paste0(fid, ".RData")) %>%
        load2() %>%
        filter(subset == "pan") %>%
        transmute(CNA=m, Pathway=scores, Size=size, Effect=estimate, `P-value`=p.value, FDR=adj.p) %>%
        tab(caption=paste(caption, "vs. CNAs (pan-cancer)"), p=0.05, n=50, ...)
}
do("gsva_go", "Gene Ontology")
do("gsva_reactome", "Reactome")
do("spia", "SPIA")
...
do = function(fid, caption="", ...) {
    file.path(speed2, "drug_assocs/assocs_mapped", paste0(fid, ".RData")) %>%
        load2("assocs.pan") %>%
        transmute(Drug=drug, Pathway=scores, Size=size, Effect=estimate, `P-value`=p.value, FDR=adj.p) %>%
...
do = function(fid, caption="", ...) {
    file.path(speed2, "drug_assocs/assocs_mapped", paste0(fid, ".RData")) %>%
        load2("assocs.tissue") %>%
        filter(subset == "noexp") %>%
        transmute(Drug=drug, Pathway=scores, Tissue=tissue, Size=size, Effect=estimate, `P-value`=p.value, FDR=adj.p) %>%
...
do = function(caption="", ...) {
    file.path(mantra, "assocs.RData") %>%
        load2("pan") %>%
        transmute(`Drug response`=drug_resp, Sensitiser=scores, Size=size, Effect=estimate, `P-value`=p.value, FDR=p.adjust(p.value, method="fdr")) %>%
...
do = function(caption="", ...) {
    file.path(mantra, "assocs.RData") %>%
        load2("tissue") %>%
        transmute(`Drug response`= drug_resp, Sensitiser=scores, Type=subset, Size=size, Effect=estimate, `P-value`=p.value, FDR=p.adjust(p.value, method="fdr")) %>%
        tab(caption=caption, p=0.1, n=50)
}
