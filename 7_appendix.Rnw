...
do = function(fid, caption="", subset="assocs.pan", ...) {
...
{\tiny
<<pan_drug_all_speed1, echo=FALSE>>=
...
\clearpage
\section{Associations for evaluating perturbation-response signatures (chapter 4)\label{sec:ch4}}
\setcounter{table}{0}
...
\par\end{centering}
\caption{Predicted vs. measured synergy for drug combinations}
\label{fig:syn}
\end{figure}
